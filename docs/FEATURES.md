...
| `add`           | `{title, dateISO, rank, repeat?}` | `{ok, task}`                          | dateISO เป็น date-only หรือ datetime ก็ได้; repeat = ทำซ้ำ |
| `complete`      | `{pageId, rank}`               | `{ok, reward, remaining, game}`          | ให้ XP + อัปเดต streak ถ้าเคลียร์ครบวัน |
| `completeRecurring`| `{pageId, rank, repeat, date}` | `{ok, reward, remaining, game, next}` | "เสร็จวันนี้" ของ recurring: เลื่อนไป occurrence ถัดไป + XP (ไม่ติ๊ก done) |
...
### Recurring quest (งานทำซ้ำ) — v0.4.0
- **ตั้ง**: quick-add พิมพ์ "ทุกวัน / ทุกสัปดาห์ (หรือ ทุกอาทิตย์) / ทุกเดือน" — `parseQuickAdd` แยกออกจากชื่อ
  คืน field `repeat` เก็บลง Notion select `propMap.repeat` (default "ทำซ้ำ", options ใน `questSchema`)
- **action 2 ปุ่ม** (popup `renderList` + quest `makeCard` แสดงเมื่อ `t.repeat`):
  - **เสร็จวันนี้** → `completeRecurring`: `nextOccurrence(date, repeat, today)` เลื่อน `date.start` ไป
    occurrence ถัดไป (ผ่าน `snoozeTask`) + `applyReward` (ได้ XP) — ไม่ติ๊ก done งานจึงโผล่อีกครั้งหน้า
  - **ปิดงาน** → `complete` ปกติ: ติ๊ก done = หยุดทำซ้ำถาวร
- `nextOccurrence(dateISO, repeat, todayISO)` (`thaiDate.js`): เดินทีละ period (วัน+1 / สัปดาห์+7 /
  เดือน `addMonths(+1)` clamp สิ้นเดือน) **จากวันเดิม** วน `do..while (day <= today)` จนเลยวันนี้ —
  รักษา anchor (วันในสัปดาห์/วันที่ของเดือน) แม้ overdue, คงส่วนเวลา datetime ไว้
- schema: `QUEST_SCHEMA_VERSION = 2` + `QUEST_SCHEMA_RELEASES[2]` — user เดิมกดอัปเดต schema ที่หน้า migrate
  เพื่อเพิ่ม column "ทำซ้ำ"
- test: `nextOccurrence` / `addMonths` / repeat parsing ใน `test/thaiDate.test.mjs`
...
### เวอร์ชัน
- options.js โชว์ `v<getManifest().version>` ที่ `#version-label` หัวหน้า
- **เช็คเวอร์ชันใหม่จาก GitHub ถอดออกแล้ว (v0.3.0)** — เคยมี `lib/version.js` + action `checkUpdate`
  + แบนเนอร์ popup/options แต่ใช้จริงไม่เวิร์ค ลบทิ้ง ดู CLAUDE.md → Versioning
...
```js
{ id, title, date, rank, repeat, url }
// date   = date.start ดิบจาก Notion: "2026-06-17" หรือ "2026-06-17T12:00:00+07:00"
// repeat = "ทุกวัน" | "ทุกสัปดาห์" | "ทุกเดือน" | null (recurring quest)
